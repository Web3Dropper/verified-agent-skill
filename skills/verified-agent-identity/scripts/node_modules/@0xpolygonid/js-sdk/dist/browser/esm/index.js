...
var W3CCredential = class _W3CCredential {
  ...
  async verifyBJJSignatureProof(proof, coreClaim, resolverURL, userDID, credStatusResolverRegistry) {
    ...
    const pubKey = new PublicKey2([rawSlotsInt[2], rawSlotsInt[3]]);
...
var buildSolanaPayment = async (solSigner, option, chainId, paymentRails, recipient, amount, expirationDate, nonce) => {
  ...
  if (option.type === "Iden3PaymentRailsSolanaRequestV1" /* Iden3PaymentRailsSolanaRequestV1 */) {
    const request = new SolanaNativePaymentRequest({
      ...
      verifyingContract: new PublicKey3(paymentRails).toBytes(),
      recipient: new PublicKey3(recipient).toBytes(),
    ...
    const request = new SolanaSplPaymentRequest({
      ...
      chainId: BigInt(chainId),
      verifyingContract: new PublicKey3(paymentRails).toBytes(),
      tokenAddress: new PublicKey3(option.contractAddress).toBytes(),
      recipient: new PublicKey3(recipient).toBytes(),
      amount: BigInt(amount),
      expirationDate: BigInt(getUnixTimestamp(expirationDate)),
      nonce,
...
var serializeSolanaPaymentInstruction = (data) => {
  ...
  if (data.type === "Iden3PaymentRailsSolanaRequestV1" /* Iden3PaymentRailsSolanaRequestV1 */) {
    const request = new SolanaNativePaymentRequest({
      ...
      verifyingContract: new PublicKey3(proof.domain.verifyingContract).toBytes(),
      recipient: new PublicKey3(data.recipient).toBytes(),
    ...
    const request = new SolanaSplPaymentRequest({
      ...
      chainId: BigInt(proof.domain.chainId),
      verifyingContract: new PublicKey3(proof.domain.verifyingContract).toBytes(),
      tokenAddress: new PublicKey3(data.tokenAddress).toBytes(),
      recipient: new PublicKey3(data.recipient).toBytes(),
      amount: BigInt(data.amount),
      expirationDate: BigInt(getUnixTimestamp(new Date(data.expirationDate))),
      nonce: BigInt(data.nonce),
...
var verifyIden3SolanaPaymentRequest = async (data, resolver) => {
  ...
  return ed25519.verify(proof.proofValue, serialized, new PublicKey3(publicKeyMultibase).toBytes());
};

// src/utils/payments/evm.ts
import { ethers } from "ethers";
import { getUnixTimestamp as getUnixTimestamp2 } from "@iden3/js-iden3-core";
var buildEvmPayment = async (signer, option, chainId, paymentRails, recipient, amount, expirationDateRequired, nonce) => {
  ...
  const paymentData = option.type === "Iden3PaymentRailsRequestV1" /* Iden3PaymentRailsRequestV1 */ ? {
    recipient,
    amount,
    expirationDate: getUnixTimestamp2(expirationDateRequired),
    nonce,
    metadata: "0x"
  } : {
    tokenAddress: option.contractAddress,
...
async function verifyEIP712TypedData(data, resolver) {
  const paymentData = data.type === "Iden3PaymentRailsRequestV1" /* Iden3PaymentRailsRequestV1 */ ? {
    ...
    amount: data.amount,
    expirationDate: getUnixTimestamp2(new Date(data.expirationDate)),
    nonce: data.nonce,
    metadata: "0x"
  } : {
...
var BjjProvider = class {
  ...
  async sign(keyId, data) {
    ...
    const i = BytesHelper2.bytesToInt(data);
  ...
  async privateKey(keyId) {
    ...
    return new PrivateKey(Hex4.decodeString(privateKeyHex));
  }
  async verify(message, signatureHex, keyId) {
    const publicKey = await this.publicKey(keyId);
    return PublicKey4.newFromCompressed(hexToBytes(publicKey)).verifyPoseidon(
      BytesHelper2.bytesToInt(message),
      Signature2.newFromCompressed(hexToBytes(signatureHex))
    );
  }
};
...
var Ed25519Provider = class {
  ...
  async newPrivateKeyFromSeed(seed) {
    ...
    const publicKey = ed255192.getPublicKey(seed);
    const kmsId = {
      type: this.keyType,
  ...
  async publicKey(keyId) {
    ...
    const publicKey = ed255192.getPublicKey(privateKeyHex);
  ...
  async sign(keyId, digest) {
    const privateKeyHex = await this.privateKey(keyId);
    return ed255192.sign(digest, privateKeyHex);
  }
  /**
  ...
  async verify(digest, signatureHex, keyId) {
    ...
    return ed255192.verify(signatureHex, digest, publicKeyHex);
  }
  /**
...
var defaultRSAOaepKmsIdPathGeneratingFunction = (publicKey) => {
  ...
  return ethers2.keccak256(pathBytes);
};
...
import {
  ...
  BytesHelper as BytesHelper10,
  Claim as Claim9,
  ClaimOptions as ClaimOptions2,
  DID as DID27,
  DidMethod,
  getUnixTimestamp as getUnixTimestamp10,
  Id as Id19,
  NetworkId,
...
var AtomicQueryV3PubSignals = class extends BaseConfig {
  ...
  pubSignalsUnmarshal(data) {
    ...
    fieldIdx++;
    this.userID = Id6.fromBigInt(BigInt(pubSignals[fieldIdx]));
    fieldIdx++;
    this.issuerState = Hash9.fromString(pubSignals[fieldIdx]);
    fieldIdx++;
    ...
    fieldIdx++;
    this.issuerID = Id6.fromBigInt(BigInt(pubSignals[fieldIdx]));
    fieldIdx++;
    this.isRevocationChecked = parseInt(pubSignals[fieldIdx]);
    fieldIdx++;
    this.issuerClaimNonRevState = Hash9.fromString(pubSignals[fieldIdx]);
    fieldIdx++;
    this.timestamp = parseInt(pubSignals[fieldIdx]);
    fieldIdx++;
    this.claimSchema = SchemaHash5.newSchemaHashFromInt(BigInt(pubSignals[fieldIdx]));
    fieldIdx++;
    ...
    if (pubSignals[fieldIdx] !== "0") {
      this.verifierID = Id6.fromBigInt(BigInt(pubSignals[fieldIdx]));
    }
    fieldIdx++;
...
var circuitValidator = {
  ...
  ["credentialAtomicQueryMTPV2OnChain" /* AtomicQueryMTPV2OnChain */]: credentialAtomicQueryV2OnChainValidation,
  ["credentialAtomicQuerySigV2" /* AtomicQuerySigV2 */]: credentialAtomicQueryV2Validation,
  ["credentialAtomicQuerySigV2OnChain" /* AtomicQuerySigV2OnChain */]: credentialAtomicQueryV2OnChainValidation,
  ["credentialAtomicQueryV3-beta.1" /* AtomicQueryV3 */]: credentialAtomicQueryV3Validation,
  ["credentialAtomicQueryV3OnChain-beta.1" /* AtomicQueryV3OnChain */]: credentialAtomicQueryV3Validation,
  ["authV2" /* AuthV2 */]: noQueriesValidation,
  ["authV3" /* AuthV3 */]: noQueriesValidation,
  ["authV3-8-32" /* AuthV3_8_32 */]: noQueriesValidation,
...
var packZkpProof = (inputs, a, b, c) => {
  return new ethers3.AbiCoder().encode(
...
var createZkpRequestCacheKey = (version, profileDID, r, credId) => {
  ...
  return `${version}:${profileDID.string()}:${credId}:${sha2562(requestCanonicalBytes)}`;
};
...
import { p384 as p3842 } from "@noble/curves/p384";
import { BytesHelper as BytesHelper4 } from "@iden3/js-iden3-core";
...
var toPublicKeyJwk = (keyStr, keyType) => {
  switch (keyType) {
    ...
    case "P-384" /* P384 */: {
      const pubJwk = p3842.Point.fromHex(keyStr);
      const coordinateByteLength = 48;
      const xBytes = BytesHelper4.intToNBytes(pubJwk.x, coordinateByteLength).reverse();
      const yBytes = BytesHelper4.intToNBytes(pubJwk.y, coordinateByteLength).reverse();
...
import { BytesHelper as BytesHelper5, DID as DID6 } from "@iden3/js-iden3-core";
    ...
    case "authV3" /* AuthV3 */:
      {
        ...
        const challenge = BytesHelper5.bytesToInt(msgHash.reverse());
...
import { ProvingMethodAlg as ProvingMethodAlg2 } from "@iden3/js-jwz";
var DefaultZKPPacker = class extends ZKPPacker {
  ...
  async unpack(envelope) {
    ...
    const provingMethodAlg = new ProvingMethodAlg2(token.alg, circuitId);
...
import { DID as DID8, getUnixTimestamp as getUnixTimestamp4 } from "@iden3/js-iden3-core";
import { proving as proving3 } from "@iden3/js-jwz";
import * as uuid2 from "uuid";

// src/iden3comm/handlers/common.ts
import { getRandomBytes } from "@iden3/js-crypto";
import { getUnixTimestamp as getUnixTimestamp3 } from "@iden3/js-iden3-core";
import { ethers as ethers4 } from "ethers";

// src/iden3comm/handlers/message-handler.ts
import { Token as Token2, proving as proving2 } from "@iden3/js-jwz";
var defaultProvingMethodAlg = proving2.provingMethodGroth16AuthV2Instance.methodAlg;
...
async function getProvingMethodAlgFromJWZ(bytes) {
  try {
    ...
    const token = await Token2.parse(tokenString);
...
var packMetadatas = (metas) => {
  return new ethers4.AbiCoder().encode(["tuple(string key,bytes value)[]"], [metas]);
};
var verifyExpiresTime = (message) => {
  if (message?.expires_time && message.expires_time < getUnixTimestamp3(/* @__PURE__ */ new Date())) {
...
var AuthHandler = class extends AbstractMessageHandler {
  ...
  getDefaultProvingMethodAlg(preferredAuthProvingMethod, accept) {
    ...
    const preferredOrder = [
      proving3.provingMethodGroth16AuthV3_8_32Instance.methodAlg,
      proving3.provingMethodGroth16AuthV3Instance.methodAlg
    ];
...
import { buildDIDType as buildDIDType2, BytesHelper as BytesHelper6, DID as DID10, Id as Id11 } from "@iden3/js-iden3-core";
...
function isIssuerGenesis(issuer, state) {
  ...
  const arr = BytesHelper6.hexToBytes(state);
  const stateBigInt = BytesHelper6.bytesToInt(arr);
...
var FetchHandler = class _FetchHandler extends AbstractMessageHandler {
  ...
  async handleCredentialOffer(offer, opts) {
    ...
    const packerOptions = initDefaultPackerOptions(mediaType, opts?.packerOptions, {
      ...
      senderDID: DID12.parse(offerMessage.to)
    });
...
import { DID as DID13, ChainIds, getUnixTimestamp as getUnixTimestamp5, BytesHelper as BytesHelper7 } from "@iden3/js-iden3-core";
var ContractRequestHandler = class extends AbstractMessageHandler {
  ...
  async handleContractInvoke(message, ctx) {
    ...
    const zkpResponses = await processZeroKnowledgeProofRequests(
      ...
      {
        ethSigner,
        challenge: challenge ?? BytesHelper7.bytesToInt(hexToBytes(await ethSigner.getAddress())),
        supportedCircuits: this._supportedCircuits
      }
    );
...
import { DID as DID16, getUnixTimestamp as getUnixTimestamp6 } from "@iden3/js-iden3-core";
...
function createProposalRequest(sender, receiver, opts) {
  ...
  const request = {
    ...
    expires_time: opts?.expires_time ? getUnixTimestamp6(opts.expires_time) : void 0,
    attachments: opts.attachments
  };
  return request;
}
...
import { DID as DID17, getUnixTimestamp as getUnixTimestamp7 } from "@iden3/js-iden3-core";
...
function createPaymentRequest(sender, receiver, agent, payments, opts) {
  ...
  const request = {
    ...
    created_time: getUnixTimestamp7(/* @__PURE__ */ new Date()),
    expires_time: opts?.expires_time ? getUnixTimestamp7(opts.expires_time) : void 0
  };
  return request;
}
...
import { getUnixTimestamp as getUnixTimestamp8 } from "@iden3/js-iden3-core";
function createDiscoveryFeatureQueryMessage(queries, opts) {
  ...
  return {
    ...
    from: opts?.from,
    to: opts?.to,
    created_time: getUnixTimestamp8(/* @__PURE__ */ new Date()),
  ...
  async handleDiscoveryQuery(message, opts) {
    ...
    return Promise.resolve(
      createDiscoveryFeatureDiscloseMessage(disclosures, {
        to: message.from,
        from: message.to,
        expires_time: opts?.disclosureExpiresDate ? getUnixTimestamp8(opts.disclosureExpiresDate) : void 0
      })
    );
  }
  handleQuery(query) {
...
var JoseService = class {
  ...
  async decrypt(data) {
    ...
    if (Object.prototype.hasOwnProperty.call(data, "encrypted_key")) {
      ...
      return flattenedDecrypt(data, getKey);
    }
    return generalDecrypt(data, getKey);
  }
...
var EthStateStorage = class {
  ...
  async getStateInfoByIdAndState(id, state) {
    ...
    try {
      ...
      const isGenesis = getIsGenesisStateById(Id12.fromBigInt(id), state);
      if (!isGenesis) {
        throw new Error(
...
import { JsonRpcProvider as JsonRpcProvider3, Contract as Contract3, ethers as ethers5 } from "ethers";
...
import { BytesHelper as BytesHelper8, chainIDfromDID, DID as DID19, Id as Id13 } from "@iden3/js-iden3-core";
...
var OnChainZKPVerifier = class _OnChainZKPVerifier {
  ...
  static _supportedCircuitsPubSignalsMap = {
    ["credentialAtomicQueryMTPV2OnChain" /* AtomicQueryMTPV2OnChain */]: { ctor: AtomicQueryMTPV2OnChainPubSignals },
    ["credentialAtomicQuerySigV2OnChain" /* AtomicQuerySigV2OnChain */]: { ctor: AtomicQuerySigV2OnChainPubSignals },
    ["credentialAtomicQueryV3OnChain-beta.1" /* AtomicQueryV3OnChain */]: { ctor: AtomicQueryV3OnChainPubSignals },
    ["credentialAtomicQueryV3OnChain" /* AtomicQueryV3OnChainStable */]: { ctor: AtomicQueryV3OnChainPubSignals },
    ["authV2" /* AuthV2 */]: { ctor: AuthV2PubSignals },
    ["authV3" /* AuthV3 */]: { ctor: AuthV3PubSignals },
    ["authV3-8-32" /* AuthV3_8_32 */]: { ctor: AuthV3PubSignals, opts: { mtLevel: 8, mtLevelClaim: 32 } }
  };
  ...
  static packCrossChainProofs(gistUpdateArr, stateUpdateArr) {
    ...
    return new ethers5.AbiCoder().encode(
...
var packEthIdentityProof = (did) => {
  return `0x${bytesToHex(BytesHelper8.intToBytes(DID19.idFromDID(did).bigInt()))}`;
};
...
import { Contract as Contract4, ethers as ethers7 } from "ethers";
...
var OnchainNonMerklizedIssuerAdapter = class {
  ...
  constructor(ethConnectionConfig, issuerDid, options) {
    ...
    this._contractAddress = ethers6.getAddress(
      ethers6.hexlify(Id14.ethAddressFromId(DID20.idFromDID(issuerDid)))
    );
    this._contract = NonMerklizedIssuerBase__factory.connect(
      this._contractAddress,
      new ethers6.JsonRpcProvider(ethConnectionConfig.url)
    );
...
var OnchainIssuer = class {
  ...
  getContractConnection(did) {
    ...
    const contractAddress = ethers7.getAddress(ethers7.hexlify(Id15.ethAddressFromId(issuerId)));
    ...
    const contract = new Contract4(contractAddress, abi, new ethers7.JsonRpcProvider(connection.url));
...
import { DID as DID23, getUnixTimestamp as getUnixTimestamp9 } from "@iden3/js-iden3-core";
var InputGenerator = class {
  ...
  credentialAtomicQueryMTPV2PrepareInputs = async ({
    ...
    circuitInputs.currentTimeStamp = getUnixTimestamp9(/* @__PURE__ */ new Date());
...
function calculateGroupId() {
  const groupID = BigInt(ethers9.keccak256(ethers9.randomBytes(32))) & // It should fit in a field number in the circuit (max 253 bits). With this we truncate to 252 bits for the group ID
  BigInt("0x0FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF");
  return groupID;
}
function calculateRequestId(requestParams, creatorAddress) {
  const requestId = (BigInt(
    ethers9.keccak256(ethers9.solidityPacked(["bytes", "address"], [requestParams, creatorAddress]))
...
var ProofService = class {
  ...
  async generateAuthProof(circuitId, identifier, opts) {
    ...
    const challenge = opts.challenge ? BytesHelper9.intToBytes(opts.challenge).reverse() : new Uint8Array(32);
  ...
  async generateAuthV2Inputs(hash, did, circuitId) {
    ...
    const challenge = BytesHelper9.bytesToInt(hash.reverse());
  ...
  async generateAuthInputs(hash, did, circuitId) {
    ...
    if (circuitId === "authV3-8-32" /* AuthV3_8_32 */) {
...
var IdentityWallet = class {
  ...
  async createAuthCoreClaim(revNonce, seed) {
    ...
    const pubKey = PublicKey5.newFromHex(pubKeyHex);
  ...
  async createAuthBJJCredential(did, pubKey, authClaim, currentState, revocationOpts) {
    ...
    const expiration = authData ? getUnixTimestamp10(authData) : 0;
    const request = {
  ...
  async createIdentity(opts) {
    const tmpIdentifier = opts.seed ? uuid14.v5(Hex6.encode(sha2563(opts.seed)), uuid14.NIL) : uuid14.v4();
  ...
  async sign(message, credential) {
    ...
    const signature = await this._kms.sign(keyKMSId, BytesHelper10.intToBytes(payload));
  ...
  async signChallenge(challenge, credential) {
    const keyKMSId = getKMSIdByAuthCredential(credential);
    const signature = await this._kms.sign(keyKMSId, BytesHelper10.intToBytes(challenge));
    return Signature3.newFromCompressed(signature);
  }
...
export {
  ...
  keyPath,
  mergeObjects,
  notifications_exports as notification,
