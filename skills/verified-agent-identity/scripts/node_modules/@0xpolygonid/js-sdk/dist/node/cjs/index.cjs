...
var verifyIden3SolanaPaymentRequest = async (data, resolver) => {
  ...
  return import_ed25519.ed25519.verify(proof.proofValue, serialized, new import_web3.PublicKey(publicKeyMultibase).toBytes());
};
...
var Ed25519Provider = class {
  ...
  async publicKey(keyId) {
    ...
    const publicKey = import_ed255192.ed25519.getPublicKey(privateKeyHex);
  ...
  async sign(keyId, digest) {
    const privateKeyHex = await this.privateKey(keyId);
    return import_ed255192.ed25519.sign(digest, privateKeyHex);
  }
  /**
  ...
  async verify(digest, signatureHex, keyId) {
    ...
    return import_ed255192.ed25519.verify(signatureHex, digest, publicKeyHex);
  }
  /**
...
var Sec256k1Provider = class {
  ...
  async verify(message, signatureHex, keyId) {
    const publicKeyHex = await this.publicKey(keyId);
    return import_secp256k1.secp256k1.verify(signatureHex, (0, import_js_crypto7.sha256)(message), publicKeyHex);
  }
  async privateKey(keyId) {
    return this._keyStore.get({ alias: keyId.id });
  }
...
var P384Provider = class {
  ...
  async newPrivateKeyFromSeed(seed) {
    ...
    const publicKey = import_p384.p384.getPublicKey(seed);
    const kmsId = {
      type: this.keyType,
      id: keyPath(this.keyType, bytesToHex(publicKey))
    };
  ...
  async publicKey(keyId) {
    const privateKeyHex = await this.privateKey(keyId);
    const publicKey = import_p384.p384.getPublicKey(privateKeyHex);
    return bytesToHex(publicKey);
  }
  /**
   * signs prepared payload of size,
  ...
  async sign(keyId, digest) {
    const privateKeyHex = await this.privateKey(keyId);
    const signature = import_p384.p384.sign(digest, privateKeyHex);
  ...
  async verify(digest, signatureHex, keyId) {
    const publicKeyHex = await this.publicKey(keyId);
    return import_p384.p384.verify(signatureHex, digest, publicKeyHex);
  }
  /**
   * Retrieves the private key for a given keyId from the key store.
   * @param {KmsKeyId} keyId - The identifier of the key to retrieve.
...
var import_p3842 = require("@noble/curves/p384");
...
var toPublicKeyJwk = (keyStr, keyType) => {
  switch (keyType) {
    ...
    case "P-384" /* P384 */: {
      const pubJwk = import_p3842.p384.Point.fromHex(keyStr);
...
var ProofService = class {
  ...
  async generateAuthV2Inputs(hash, did, circuitId) {
    ...
    const id = import_js_iden3_core42.DID.idFromDID(genesisDID);
    const stateProof = await this._stateStorage.getGISTProof(id.bigInt());
    const gistProof = toGISTProof(stateProof);
